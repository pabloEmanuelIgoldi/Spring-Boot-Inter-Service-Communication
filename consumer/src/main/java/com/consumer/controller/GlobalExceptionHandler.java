...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiResponseDTO<String>> handleUnauthorizedException(UnauthorizedException e) {
		...
		return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
				.body(new ApiResponseDTO<>(false, MensajeResponseUtil.UNAUTHORIZED,
						CodeResponseUtil.UNAUTHORIZED, LocalDateTime.now(), null));
	}
	
	@ExceptionHandler(BadRequestException.class)
	public ResponseEntity<ApiResponseDTO<Map<String, String>>> handleBadRequestException(
			BadRequestException ex) {
		log.error("GlobalExceptionHandler.handleBadRequestException(): " + ex);
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiResponseDTO<Map<String, String>>(false,
				MensajeResponseUtil.BAD_REQUEST, CodeResponseUtil.BAD_REQUEST, LocalDateTime.now(), null));
	}
	...
	public ResponseEntity<ApiResponseDTO<String>> handleServiceException(ServiceException e) {
		log.error("GlobalExceptionHandler.handleServiceException(): " + e);
		return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(new ApiResponseDTO<>(false,
				MensajeResponseUtil.SERVICE_UNAVAILABLE, CodeResponseUtil.SERVICE_UNAVAILABLE, LocalDateTime.now(), null));
	}
}
